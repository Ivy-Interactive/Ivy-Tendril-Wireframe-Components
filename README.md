# Tendril — hand-drawn wireframe components

A React component library drawn as if someone sketched it on paper. Think Balsamiq:
wobbly borders, hatched fills, a handwriting typeface and a deliberately unfinished feel,
so nobody mistakes a wireframe for a finished design.
...
runtime, which means the whole icon map has to be in the bundle and cannot be
tree-shaken. It is the single largest part of the package. If that matters to you, import
lucide components directly and pass them as children instead of using the `icon` prop.

## The prop surface

Props are named enums rather than booleans and utility classes: `variant="Destructive"`,
`density="Small"`, `borderRadius="Full"`, `formatStyle="Currency"`,
`colorScheme="Rainbow"`. Events are ordinary React callbacks — `onClick`, `onChange`,
`onSelect`, `onOpenChange` — and composite widgets take named `ReactNode` props for their
regions (`header`, `children`, `footer`) rather than a slot object.

Every component also takes a shared base: `width`, `height`, `aspectRatio`, `density`,
`visible`, `id`, plus `className`, `style` and `data-testid`. `visible={false}` hides a
widget without unmounting it. Sizes accept CSS lengths (`"20rem"`), fractions (`"1/2"`)
and bare numbers, which mean quarter-rem steps. `density` defaults to `Medium`
everywhere.

Tendril covers the layouts that are worth drawing by hand — `TabsLayout`,
`SidebarLayout`, `ResizablePanelGroup` and `FloatingPanel`. Purely structural layout has
no equivalent here, since flexbox and CSS grid already say the same thing with less.

### Component index

**Primitives** — `TextBlock`, `Markdown`, `Json`, `Xml`, `CodeBlock`, `Terminal`,
`Embed`, `Separator`, `Skeleton`, `Icon`, `Box`, `Callout`, `Kbd`, `Avatar`, `Loading`,
`Stepper`, `AudioPlayer`, `VideoPlayer`

`CodeBlock` and `CodeInput` colour their own syntax — keywords, strings, numbers and
comments, in the same desaturated range as the rest of the palette, from a small built-in
tokenizer rather than a highlighting dependency.
...
`Chat` + `ChatMessage` + `ChatLoading` + `ChatStatus`, `Kanban` + `KanbanCard`

**Layouts** — `TabsLayout` + `Tab`, `SidebarLayout` + `SidebarMenu`, `ResizablePanelGroup`
+ `ResizablePanel`, `FloatingPanel`

**Wireframe marks** — `WireframeNote`, `WireframeCallout`, `WireframePlaceholder`,
`WireframeArrow`, `WireframeCurlyBrace`, `WireframeRedX`, `WireframeScratchOut`,
`WireframeMockup`, `WireframeTransform`

`WireframeMockup` wraps real content in a phone, tablet, desktop-window or browser frame;
`WireframeTransform` rotates, scales, skews or flips a subtree without disturbing its
neighbours. `WireframeShape` is the one wireframe widget with no Tendril equivalent yet.
...
geometry matches the rest of the sketch. Each chart takes its series as an array
(`lines`, `bars`, `areas`, `scatters`, `pies`, `radars`, `funnels`) alongside `xAxis`,
`yAxis` and `colorScheme`.
...
defaults, which props are events, which are inherited — so a code generator, a design
tool or an LLM tool schema can enumerate the library without keeping its own copy of the
API.
...
drift from the code. Prose — category, tags, examples — comes from JSDoc tags in the
component files:
...
src/
  ...
  widgets/      the components, grouped by kind
    primitives/
    inputs/
    charts/
    layouts/
    wireframe/
  stories/      Storybook catalogue — one section per component, mirroring
                the widget folders (widgets/, inputs/, charts/, layouts/,
                wireframe/, primitives/), plus shared fixtures and helpers
